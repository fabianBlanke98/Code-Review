...
export const rawKeyFor = (albumId: string, clipId: string, revision = 1): string =>
  `albums/${albumId}/raw/${clipId}-r${revision}`;
...
/**
 * Re-shoot a clip in place.
 *
 * The row already exists and already owns a slot in the film, so this only
 * swaps the footage: `replace_own_clip` bumps the revision and reopens the clip
 * for upload, and from there the path is identical to a fresh recording.
 *
 * A failure leaves the clip sitting in `uploading`, which its author still sees
 * and can retry. Deleting it on failure would throw away the slot as well as
 * the take, which is not what "vervangen" asked for.
 */
export async function replaceClip(clipId: string, pending: Omit<PendingClip, 'albumId'>): Promise<void> {
  const contentType = pending.contentType ?? 'video/mp4';

  const { error: reopenError } = await supabase.rpc('replace_own_clip', { p_clip: clipId });
